...
/*! \brief The lexer class divides a text stream into tokens. */
...
protected:
	/**
	 * \brief Internal used function to get the next token.
	 *
...
/**
 * \brief Basic parse-handler interface.
 *
 * That is, a set of callbacks getting called by the parser on the occurence of a particular statement in the config
 * file.
 *
 * \ingroup conf
 */
...
public:
	...
	virtual bool handleDigitalContact(qint64 idx, const QString &name, DigitalContact::Type type, qint64 id, bool rxTone, qint64 line, qint64 column, QString &errorMessage);
...
/**
 * \brief The actual config file parser.
 *
 * This class parses the config file and calls the associated callback functions of a handler instance that is
 * responsible to assemble the final \c Config instance.
 */
...
/**
 * \brief Implements the text-file codeplug reader.
 *
 * \ingroup conf
 */
...
public:
	/**
	 * \brief Reads a config file from \c stream and stores the read configuration into \c config.
	 *
