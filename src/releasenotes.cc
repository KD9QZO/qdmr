...
void ReleaseNotes::checkForUpdate() {
	Settings settings;
...
void ReleaseNotes::onResponse(QNetworkReply *reply) {
	if (reply->error()) {
...
void ReleaseNotes::show(const QString &notes) {
	QMessageBox::information(nullptr, tr("qDMR was updated to version %1").arg(VERSION_STRING), notes);
