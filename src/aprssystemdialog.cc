...
APRSSystemDialog::APRSSystemDialog(Config *config, QWidget *parent): QDialog(parent), _config(config), _aprs(nullptr), ui(new Ui::aprssystemdialog), _icons0(":/icons/aprs/table0.png") {
	construct();
}

APRSSystemDialog::APRSSystemDialog(Config *config, APRSSystem *aprs, QWidget *parent): QDialog(parent), _config(config), _aprs(aprs), ui(new Ui::aprssystemdialog), _icons0(":/icons/aprs/table0.png") {
	construct();
}
...
void APRSSystemDialog::construct() {
	// Construct UI
	ui->setupUi(this);
...
APRSSystem *APRSSystemDialog::aprsSystem() {
	if (_aprs) {
	...
	return (new APRSSystem(ui->name->text().simplified(), ui->channel->currentData().value<AnalogChannel*>(), ui->destination->text().simplified(), ui->destSSID->value(), ui->source->text().simplified(), ui->srcSSID->value(), ui->path->text().simplified(), APRSSystem::Icon(ui->icon->currentData().toUInt()), ui->message->text().simplified()));
}

QIcon APRSSystemDialog::aprsIcon(APRSSystem::Icon icon) {
